...
eshopControllers.controller('AdminCategoriesCtrl',
    function ($scope, $rootScope, $routeParams, $http) {
        ...
        $scope.deleteCategory = function (category) {
            console.log("deleting category with id=" + category.id + ' (' + category.name + ')');
            $http.delete(category._links.delete.href).then(
                function success(response) {
                    console.log('deleted category ' + category.id + ' on server');
                    //display confirmation alert
                    $rootScope.successAlert = 'Deleted category "' + category.name + '"';
                    //load new list of all products
                    loadAdminCategories($http, $scope);
                },
                function error(response) {
